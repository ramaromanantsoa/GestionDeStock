...
public class GetListProduit extends  MainWindow {
	...
	public static void getListOfProduit() {
		...
		try {
			...
			String requete = "SELECT p.ID_Produit, p.Nom_Produit, u.Type_Unite, p.Prix_Produit, p.Quantite_Produit, c.Nom_Categorie "
					+ "FROM Unite u INNER JOIN (Produit p INNER JOIN Categorie c ON p.ID_Categorie = c.ID_Categorie) "
					+ "ON p.ID_Unite = u.ID_Unite ORDER BY c.Nom_Categorie ASC";
